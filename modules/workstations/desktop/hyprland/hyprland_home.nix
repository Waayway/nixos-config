...
let
  ...
  modBind = combo: dispatcher: { _args = [ (inline ''mod .. " + ${combo}"'') (inline dispatcher) ]; };
  modBindO = combo: dispatcher: opts: { _args = [ (inline ''mod .. " + ${combo}"'') (inline dispatcher) opts ]; };
  keyBind = key: dispatcher: { _args = [ key (inline dispatcher) ]; };
  keyBindO = key: dispatcher: opts: { _args = [ key (inline dispatcher) opts ]; };
in
{
  ...
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      monitor = [
        { output = "desc:Acer Technologies X32Q FS 1414012CC3E00"; mode = "3840x2160@143.98"; position = "1600x0"; scale = 2.0; bitdepth = 10; } # Primary acer 4k 144
        { output = "desc:LG Electronics LG ULTRAGEAR 111MAXSP2986"; mode = "2560x1440@144.0"; position = "0x90"; scale = 1.6; } # 2nd lg 1440p
        { output = "desc:Philips Consumer Electronics Company Philips FTV 0x01010101"; mode = "3840x2160@120.0"; position = "1440x0"; scale = 2.5; bitdepth = 10; } # Philips tv downstairs
        { output = "desc:Nreal Air 2 Pro 0x88888800"; mode = "1920x1080@120"; position = "auto"; scale = 1; }
      ];
      ...
      curve = [
        { _args = [ "myBezier" { type = "bezier"; points = [ [ 0.05 0.9 ] [ 0.1 1.1 ] ]; } ]; }
        { _args = [ "overshot" { type = "bezier"; points = [ [ 0.05 0.9 ] [ 0.1 1.01 ] ]; } ]; }
      ];

      # Per-leaf animations (old `animation = name, onoff, speed, curve[, style]`).
      animation = [
        { leaf = "windows"; enabled = true; speed = 7; curve = "myBezier"; }
        { leaf = "windowsOut"; enabled = true; speed = 7; curve = "default"; style = "slide"; }
        { leaf = "windowsIn"; enabled = true; speed = 5; curve = "myBezier"; style = "slide"; }
        { leaf = "windowsMove"; enabled = true; speed = 5; curve = "default"; }
        { leaf = "border"; enabled = true; speed = 10; curve = "default"; }
        { leaf = "borderangle"; enabled = true; speed = 8; curve = "default"; }
        { leaf = "fade"; enabled = true; speed = 7; curve = "default"; }
        { leaf = "workspaces"; enabled = true; speed = 5; curve = "overshot"; }
      ];

      window_rule = [
        { match = { class = "^(spotify)$"; }; workspace = "name:󰝚"; }
        { match = { class = "^(discord)$"; }; workspace = "name:󰭹"; }
        { match = { class = "^(.gamescope-wrapped)$"; }; workspace = "name:󰊴"; }
      ];
      ...
      bind =
        ...
        ++ (
          ...
          builtins.concatLists (
            builtins.genList (
              ...
              in
              [
                (modBind ws ''hl.workspace(${toString n})'')
        ...
        ++ [
          (modBind "S" ''hl.workspace("name:󰝚")'')
          (modBind "D" ''hl.workspace("name:󰭹")'')
          (modBind "G" ''hl.workspace("name:󰊴")'')

          # Volume + brightness (press-and-hold via repeating)
          (keyBindO "XF86AudioRaiseVolume" ''hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+")'' { repeating = true; })
          (keyBindO "XF86AudioLowerVolume" ''hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-")'' { repeating = true; })
